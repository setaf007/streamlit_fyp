...
string_cols_limited = tuple(
    df.select_dtypes(include=["object"]).loc[
        :, df.select_dtypes(include=["object"]).nunique() < 20
    ]
)
...
def visualization(plot_selected, df):
    ...
    if plot_selected == "BoxPlot":
        ...
        if usage_info:
            ...
            st.write(
                "- This can help identify outliers, skewness, and other patterns in the data that may not be immediately apparent from summary statistics."
            )
            st.write(
                "- Can show the median, quartiles, and outliers of data collected over a period"
            )
            st.write(
                "- Hover over different parts of the boxplot to see specific values"
            )
        try:
            option = st.selectbox(
            ...
            if add_cat:
                option2 = st.selectbox(
                    "Which column boxplot would you like to use as the categories",
                    string_cols_limited,
                )
                fig = px.box(
                    df, y=option, color=option2, color_discrete_sequence=["#636EFA"]
                )
                fig.update_layout(
                    title="Boxplot",
                    yaxis_title=option,
                    ...
                    xaxis=dict(tickangle=-45, title="", tickfont=dict(size=14)),
    ...
    # PIECHART
        ...
        if usage_info:
            ...
            st.write(
                "- Example, pie chart can be used to show the percentage of rainy days vs. sunny days"
            )
        try:
            option = st.selectbox(
                "Which column piechart would you like to observe", piechart_cols(df)
            )
            new_df = df.groupby([option])[option].count().reset_index(name="count")
            st.dataframe(new_df)
    ...
    # SCATTERPLOT
        ...
        usage_info = st.checkbox("Plot usage info")
        if usage_info:
            st.write("- Scatter plot displays the relationship between two variables")
            st.write(
                "- Example, the x-axis can represent temperature, while the y-axis represents precipitation. The scatter plot will then plot each data point in the dataset as a point on the plot"
            )
            st.write(
                "- Help to identify any patterns or trends in the data, example, if there is a positive correlation between 2 variables, then the data points on the scatter plot will form a roughly upward-sloping line"
            )
        try:
            ...
            if add_cat:
                option1 = st.selectbox(
                    "Which column would you like to use as the category seperator",
                    string_cols_limited,
                )
                fig = px.scatter(
                    df, x=x_col, y=y_col, hover_name=option1, color=option1
                )
            else:
    ...
    # LINEPLOT
        ...
        usage_info = st.checkbox("Plot usage info")
        if usage_info:
            st.write("- Useful for showing trends over time.")
        ...
        try:
            x_col = st.selectbox(
                "Select x-axis column (preferably a date-time column):", all_cols
            )
            y_col = st.selectbox("Select y-axis column:", numeric_cols)
            add_cat = st.checkbox("Add location to seperate lines by")
            if add_cat:
                option1 = st.selectbox(
                    "Which column would you like to use as the location seperator",
                    string_cols_limited,
                )
    ...
    # VIOLINCHART
        ...
        if usage_info:
            ...
            st.write(
                "- The shape of the violin plot shows the distribution of the data, with the width of the plot showing the density of the data at that point. The height of the plot shows the range of the data."
            )
            st.write(
                "- For weather data, a violin plot could be useful in visualizing the distribution of a continuous variable, such as temperature or precipitation, across different categories, such as months or seasons."
            )
        try:
            ...
            fig.update_traces(opacity=0.7)
            fig.update_layout(title="Violin Chart", font=dict(size=14))
            st.plotly_chart(fig)
        except:
            st.write("Invalid column set")
    # HEATMAP
        ...
        if usage_info:
            ...
            st.write(
                "- Uses a color-coded matrix to represent the values of the variables, with each cell in the matrix representing the intersection between a specific value of one variable and a specific value of the other variable."
            )
        ...
        if correl_check:
            ...
            try:
                ...
                matrix = pd.pivot_table(df, values=y_col, index=x_col, aggfunc=np.mean)
                fig = px.imshow(
                    matrix, color_continuous_scale="viridis", title="Heatmap"
                )
                fig.update_layout(font=dict(family="Arial", size=12))
                st.plotly_chart(fig)
            except:
    ...
    # Scatter3D
        ...
        if usage_info:
            ...
            st.write(
                "- This plot type is useful for visualizing the relationship between three variables."
            )
            st.write(
                "- Help in identifying outliers or clusters of data points that may be of interest"
            )
        try:
            x_col = st.selectbox("Select x-axis column:", numeric_cols)
            y_col = st.selectbox("Select y-axis column:", numeric_cols)
            ...
            if add_cat:
                ...
                fig = px.scatter_3d(df, x=x_col, y=y_col, z=z_col, color=option1)
            else:
                fig = px.scatter_3d(df, x=x_col, y=y_col, z=z_col)
            fig.update_traces(opacity=0.7, marker=dict(size=2))
            fig.update_layout(
                legend=dict(title="Scatter3D Plot", orientation="h"),
                scene_camera=dict(eye=dict(x=2, y=2, z=2)),
            )
            st.plotly_chart(fig, use_container_width=True, height=2000, width=2000)
        except:
            st.write("Invalid column set")
    # BUBBLECHART
        ...
        if usage_info:
            ...
            st.write(
                "- Usefulness of a bubble chart in the context of weather forecasting or analysis could be to represent three variables such as temperature, humidity, and wind speed in a single chart"
            )
        try:
            x_col = st.selectbox("Select x-axis column:", numeric_cols)
            y_col = st.selectbox("Select y-axis column:", numeric_cols)
    ...
    # CHLORPLETH MAP
        ...
        usage_info = st.checkbox("Plot usage info")
        if usage_info:
            st.write("- Useful for visualizing spatial data on a map.")
            st.write(
                "- Choropleth maps can be used to display weather data such as temperature, rainfall, humidity, or wind speed, across regions, states, or countries"
            )
            st.write(
                "- Map can also be animated over time to show changes in temperature patterns, allowing for better understanding of weather patterns over time, by adding datetime feature."
            )
        geo_col = st.selectbox("Select geo-location column:", string_cols)
        ...
        if add_cat:
            option1 = st.selectbox("Select datetime column", all_cols)
...
if summ:
    ...
    val_counts_checkbox = st.checkbox(
        "Observe unique value counts of a selected column"
    )
    if val_counts_checkbox:
        st.write("**Value Counts of Column**")
        selected = st.selectbox(
            "Select column to observe unique value counts", filtered_df.columns
        )
...
plot_selected = st.selectbox("Which plot would you like to visualize?", plots)
